...
unsigned int __stdcall HandleClient(void* data) {
    ...
    while (1) {
        ...
        if (recvSize > 0) {
            std::cout << "> Received ( " << received << " ) and Send to " << threadCnt << " clients : " << std::endl;
            ...
            int sendCnt = 0;
            for (int i = 2; i < 100; i++) {
                if (clientThreads[i] == NULL) continue;
                else if (sendCnt == threadCnt) break;

                ClientInfo nowClientData = clientPool[i];
                SOCKET nowClientSocket = nowClientData.socket;
                //sockaddr_in nowclientAddr = nowClientData.clientAddress;
                //int threadN = nowClientData.threadNum;

                send(nowClientSocket, received, recvSize, 0);
                sendCnt++;
            }
